...
import { checkAndResetDailyUnlock, isDailyLimitReached, incrementQuestionsUnlocked } from "@/lib/phoneAuth";
import { useParams } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";
import { Clock, Trophy, Zap, Heart, ArrowLeft, RotateCcw, BookOpen } from "lucide-react";
import { AnimatedBackground } from "@/components/ui/animated-background";
import { Question, getRandomQuestions } from "@/data/questions";
import { categories } from "@/pages/Categories";
import { QuestionExplanation, CompactExplanation, ExplanationSummary } from "@/components/ui/question-explanation";
...
const Quiz = () => {
  const { categoryId, day } = useParams<{ categoryId?: string; day?: string }>();
  ...
  }, [timeLeft, isAnswered, gameOver, questions, playTimeWarning, playTimeUp, playTick]);
  ...
  const nextQuestion = async () => {
    ...
    if (user) {
      const updatedUser = await checkAndResetDailyUnlock(user.uid);
      setFirestoreUser(updatedUser);
      if (updatedUser && isDailyLimitReached(updatedUser) && currentQuestion < questions.length - 1) {
        setShowDailyLimitModal(true);
        return;
      }
    }
        ...
        <div className="relative z-40 min-h-screen p-4 pt-20">
          <div className="container mx-auto max-w-4xl">
            <div className="bg-white/10 backdrop-blur-md rounded-3xl border border-white/20 p-8 mb-8 text-center">
              <div className="mb-8">
                {gameOver ? <div className="text-6xl mb-4">💥</div> : <div className="text-6xl mb-4">🎉</div>}
                <h2 className="text-3xl font-display font-bold text-white mb-4">
                  {gameOver ? "Game Over!" : "Quiz Complete!"}
                </h2>
                <Badge className={`bg-gradient-to-r ${categoryColor} text-white px-4 py-2 mb-4`}>
                  {categoryName}
                </Badge>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-white/5 rounded-xl p-4">
                  <div className="text-2xl font-bold text-electric-400">{score}</div>
                  <div className="text-white/70 text-sm">Final Score</div>
                </div>
                <div className="bg-white/5 rounded-xl p-4">
                  <div className="text-2xl font-bold text-neon-400">{correctAnswers}</div>
                  <div className="text-white/70 text-sm">Correct Answers</div>
                </div>
                <div className="bg-white/5 rounded-xl p-4">
                  <div className="text-2xl font-bold text-magic-400">{accuracy.toFixed(0)}%</div>
                  <div className="text-white/70 text-sm">Accuracy</div>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6">
                <Button onClick={restartQuiz} className="bg-gradient-to-r from-neon-500 to-electric-500 hover:from-neon-400 hover:to-electric-400 text-white">
                  <RotateCcw className="w-4 h-4 mr-2" />
                  Play Again
                </Button>
                <Button variant="outline" className="bg-white/10 border-white/20 text-white hover:bg-white/20" onClick={goToCategories}>
                  <Zap className="w-4 h-4 mr-2" />
                  Try Other Category
                </Button>
              </div>
              <Button variant="ghost" className="text-white/70 hover:text-white" onClick={goHome}>
            ...
            <div className="bg-white/10 backdrop-blur-md rounded-3xl border border-white/20 p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-bold text-white">Learning Review</h3>
              </div>
              <ExplanationSummary questions={questions} userAnswers={userAnswers} />
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="relative z-10">
        <div className="fixed bottom-6 right-6 z-50 lg:hidden">
          <Button onClick={goToCategories} className="bg-gradient-to-r from-electric-500 to-neon-500 hover:from-electric-400 hover:to-neon-400 text-white shadow-2xl shadow-electric-500/30 rounded-full w-14 h-14 p-0 group" size="icon">
        ...
        <div className="container mx-auto max-w-4xl">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 lg:gap-0 mb-8">
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="icon" className="text-white hover:bg-white/10" onClick={goToCategories}>
                <ArrowLeft className="w-5 h-5" />
              </Button>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                <Badge className={`bg-gradient-to-r ${categoryColor} text-white px-4 py-2`}>
                  <Trophy className="w-4 h-4 mr-2" />
                  {categoryName}
                </Badge>
                <Button variant="outline" size="sm" className="bg-white/10 border-white/20 text-white hover:bg-white/20 hover:border-white/30 text-xs px-4 py-2 h-auto font-semibold transition-all duration-300 shadow-lg shadow-electric-500/20" onClick={goToCategories}>
            ...
            <div className="flex items-center gap-3 lg:gap-4 w-full lg:w-auto justify-between lg:justify-end">
              <div className="bg-gradient-to-r from-magic-500/20 to-electric-500/20 backdrop-blur-sm rounded-xl border border-magic-400/30 px-4 py-2 shadow-lg shadow-magic-500/10">
                ...
                <div className="text-white/60 text-xs text-center">Questions in bank</div>
              </div>
              <div className="flex items-center gap-1">
                {[...Array(3)].map((_, i) => (
                  <Heart key={i} className={`w-6 h-6 ${i < lives ? "text-danger-400 fill-current" : "text-white/20"}`} />
                ))}
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg border border-white/20 px-3 py-2">
                <div className="text-white font-mono text-lg font-bold text-center">{score}</div>
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-2">
              <span className="text-white/70 text-sm">{t("question")} {currentQuestion + 1} of {questions.length}</span>
              <span className="text-white/70 text-sm">{Math.round(progress)}% {t("complete")}</span>
            </div>
          ...
          <div className="flex justify-center mb-8">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 px-6 py-4">
              <div className="flex items-center gap-3">
                <Clock className={`w-6 h-6 ${timeLeft <= 10 ? "text-danger-400" : "text-electric-400"}`} />
                <div className={`text-3xl font-mono font-bold ${timeLeft <= 10 ? "text-danger-400" : "text-white"}`}>{timeLeft}s</div>
              </div>
            </div>
          </div>
          {showExplanation && (
            <div className="mb-8">
              <CompactExplanation question={question} isCorrect={selectedAnswer === question.correctAnswer} />
              {(currentQuestion + 1) % 5 === 0 && (
                <div className="mt-4 bg-gradient-to-r from-electric-500/10 to-neon-500/10 backdrop-blur-sm rounded-xl border border-electric-400/30 p-4 text-center">
                  <p className="text-white/80 text-sm mb-3">🎯 Want to try a different category?</p>
                  <Button variant="outline" size="sm" className="bg-gradient-to-r from-electric-500/20 to-neon-500/20 border-electric-400/50 text-electric-100 hover:bg-electric-500/30 hover:border-electric-400 text-xs px-4 py-2 h-auto font-semibold transition-all duration-300" onClick={goToCategories}>
                    <Zap className="w-3 h-3 mr-1" />
          ...
          {!showExplanation && (
            <div className="bg-white/10 backdrop-blur-md rounded-3xl border border-white/20 p-8 mb-8">
              <div className="flex items-center justify-between mb-6">
                <Badge className="bg-electric-500/20 border-electric-400 text-electric-100 px-3 py-1">{question.difficulty}</Badge>
                {question.category && <Badge className="bg-white/10 border-white/20 text-white/80 px-3 py-1">{question.category}</Badge>}
              </div>
              <h2 className="text-2xl md:text-3xl font-display font-bold text-white mb-8 leading-relaxed">{question.question}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {question.options.map((option, index) => {
                  let buttonClass = "bg-white/5 border-white/20 text-white hover:bg-white/10 text-left p-6 h-auto min-h-[80px] transition-all duration-200";
                  if (isAnswered) {
                    if (index === question.correctAnswer) {
                      buttonClass = "bg-neon-500/30 border-neon-400 text-white border-2 shadow-lg shadow-neon-500/25";
                    } else if (index === selectedAnswer && index !== question.correctAnswer) {
                      buttonClass = "bg-danger-500/30 border-danger-400 text-white border-2 shadow-lg shadow-danger-500/25";
                    } else {
                      buttonClass = "bg-white/5 border-white/20 text-white/50";
                    }
                  }
                  return (
                    <Button key={index} variant="outline" className={buttonClass} onClick={() => handleAnswerSelect(index)} disabled={isAnswered}>
                      <div className="flex items-center gap-4 w-full">
                        <div className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center font-bold shrink-0">{String.fromCharCode(65 + index)}</div>
              ...
              {question.tags && question.tags.length > 0 && (
                <div className="mt-6 flex flex-wrap gap-2">
                  {question.tags.slice(0, 3).map((tag, index) => (
                    <Badge key={index} className="bg-white/5 border-white/20 text-white/60 text-xs">#{tag.replace("-", " ")}</Badge>
                  ))}
                </div>
              )}
            </div>
