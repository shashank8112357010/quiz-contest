...
const Quiz = () => {
  ...
  useEffect(() => {
    // Check daily unlock and reset if needed
    const checkUnlock = async () => {
      ...
      // Load questions for the selected category with user-specific randomization
      const userId =
        user?.uid ||
    ...
    checkUnlock();
  }, [categoryId, user, startBackgroundMusic, stopBackgroundMusic]);
  ...
  const handleAnswerSelect = (answerIndex: number) => {
    ...
    if (isCorrect) {
      ...
      playCorrectAnswer(); // Enhanced clapping sound
    } else {
      setLives(lives - 1);
      playWrongAnswer(); // Enhanced "aww" sound
  ...
  const nextQuestion = async () => {
    ...
    // Increment questions unlocked in Firestore
...
// Generate anonymous user ID for non-logged users
...
const restartQuiz = async () => {
  // Check daily limit before restarting
  if (user) {
    const updatedUser = await checkAndResetDailyUnlock(user.uid);
    setFirestoreUser(updatedUser);
    if (updatedUser && isDailyLimitReached(updatedUser)) {
